...
class HandleEndpoint
{
    ...
    {
        $response = Http::post(config("api.server.$server_name.end_point") . config("api.server.$server_name.$prefix.prefix") . config("api.server.$server_name.$prefix.$route_name"), array_merge($request, [
